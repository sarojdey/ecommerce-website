      ...
      {features.map((feature, index) => (
        <div key={index} className="flex justify-center">
          <div className="flex  items-center space-x-3 w-44  md:w-auto">
            <span className="text-3xl sm:text-4xl text-[#ff4654]">
              {feature.icon}
            </span>
            <div className="flex flex-col justify-center">
              <span className="block font-semibold text-[0.8rem] sm:text-sm">
                {feature.title}
              </span>
              <span className="block text-gray-600 text-[0.7rem] sm:text-xs">
                {feature.description}
              </span>
            </div>
          </div>
